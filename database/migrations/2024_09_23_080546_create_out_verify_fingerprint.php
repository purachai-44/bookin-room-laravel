...
return new class extends Migration
{
    ...
    { 
        Schema::create('out_verify_fingerprint', function (Blueprint $table) {
            $table->id();  // ตั้งค่า primary key เป็น id
            $table->string('fingerprint_data');  // ต้องมีชนิดข้อมูลเหมือนกับในตาราง `fingerprints`
            $table->string('first_name');
            $table->string('last_name');
            $table->timestamp('time_out')->nullable();  // เพิ่มคอลัมน์ time_in ที่เป็น timestamp
    ...
    {
        Schema::dropIfExists('out_verify_fingerprint');
    }
};
